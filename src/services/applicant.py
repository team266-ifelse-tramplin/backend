...
from database.dto.apllications import ApplicationCreateDTO, ApplicationDTO, ApplicationsListWithQuantity
...
class Applicant(UserServiceBase):
    ...
    async def create_applicant(self, user_data: ApplicantCreateDTO) -> dict[str, Any]:
        ...
        async with self._db.get_session() as session:
            query = insert(Applicants).values(user_id=user_created_data["id"], **applicant_data_dict)
            await session.execute(query)
            await session.commit()
        return applicant_data_dict
        
    async def make_application(self, application_data: ApplicationCreateDTO) -> dict[str, Any]:
        applictaion_data_dict = application_data.model_dump()
        async with self._db.get_session() as session:
            query = insert(Applications).values(**applictaion_data_dict)
            await session.execute(query)
            await session.commit()
        return applictaion_data_dict
    ...
    async def edit_applicant_data(self, applicant_new_data: ApplicantEditDTO, user_id: UUID) -> tuple[dict[str, Any], dict[str, Any]]:
        applicant_new_data_dict = applicant_new_data.model_dump(exclude=["user"])
        async with self._db.get_session() as session:
            query = update(Applicants).values(**applicant_new_data_dict).where(Applicants.user_id == user_id).execution_options(synchronize_session=False)
            await session.execute(query)
            await session.commit()
            if applicant_new_data.user:
                updated_user_data = await self.edit_user(user_id, applicant_new_data.user)
    ...
    async def get_all_applications(self, user_id: UUID) -> ApplicationsListWithQuantity:
        async with self._db.get_session() as session:
            ...
            return ApplicationsListWithQuantity(
                applications=[ApplicationDTO.model_validate(item, from_attributes=True) for item in items],
                quantity=len(items)
            )
